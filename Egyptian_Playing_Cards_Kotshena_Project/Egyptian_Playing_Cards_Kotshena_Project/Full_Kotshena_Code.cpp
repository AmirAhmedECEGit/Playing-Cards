...
const int N = 4;						//defines the number of cards in one hand in games like Basra (4 or 8) recommended
	...
	void Play()
	{
		...
		while (!(Kotshena.IsEmpty()) || !(Player.IsEmpty()) || !(AI.IsEmpty()))
		{
			while (!(Player.IsEmpty()) || !(AI.IsEmpty()))
			{
				...
				cout<<"\nEstimated Complexity:\nO(1): Hash Table Access and Comparison\nO(Log(n)): AVL Tree insertion and deletion";
...
public:
	...
	void opening_game()
	{
		for(int i=0;i<N*6.5;i++) //N is the hand parimeter and number of cards intially equals 26
			{
